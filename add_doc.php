<?php
    ...
    if(!empty($_POST) && isset($_POST)){
        ...
        $requete = $connect->query("
                                    INSERT INTO panier_$us (titre, nom_doc, prix)
                                    VALUES('$titre', '$nom', '$prix');
                                    ");
...
<html lang="en">
  ...
  <head>
    ...
    <title>JPlearn -- Ajouter un Document </title>
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="inner-content">
                        <h2>Ajouter ce document au panier</h2>
                        <span>Ajouter le document à votre panier !</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-6 mx-auto shadow px-5 py-5">
                    <h4>Confirmez les informations de votre document !</h4>
                    <form action="add_doc.php" class="form rounded-5" method="post">
                        <?php
                            $connect = new PDO("mysql: host=localhost; dbname=doc_formation","root","");
                            $requete = $connect->query("SELECT * FROM info_doc WHERE id = $id;");
                            $row = $requete->fetch() 
                        ?>
                        <div class="mb-3">
                            <label for="titre" class="form-label h5">Titre: </label>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control" name="nom" id="nom">
                                <?php  
                                    echo "<option value='" .$row['nom_doc']. "'>" .$row['nom_doc']. "</option>";
                                ?>
                            </select>
                        </div>
                        <div class="mb-3">
                            <label for="prix" class="form-label h5">Prix(en FCFA): </label>
                        ...
                        <div>
                            <input type="submit" class="btn btn-secondary mt-3" value="Confirmer">
                            <a href="formation.php"><input type="button" class="btn btn-primary mt-3" value="Annuler"></a>
                        </div>
                    </form>
                </div>
            </div>
